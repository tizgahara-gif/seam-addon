...
from .seam_detection import clear_seams, find_edge_loop_candidates, mark_auto_seams, mark_longitudinal_seam_helper
...
class AUTOSEAMUV_OT_select_edge_loop_candidates(bpy.types.Operator):
    """Select quad-based edge loop candidate edges on selected mesh objects."""

    bl_idname = "autoseamuv.select_edge_loop_candidates"
    bl_label = "Select Edge Loop Candidates"
    bl_description = "Select edges that can be traced as quad-based edge loop candidates"
    bl_options = {"REGISTER", "UNDO"}
        ...
        try:
            ...
            for obj in selected_objects:
                try:
                    ...
                    candidate_edges = find_edge_loop_candidates(
                        mesh,
                        settings.edge_loop_min_length,
                        settings.closed_loops_only,
                    )
                    ...
                    mesh.update()
                    selected_edge_count += len(candidate_edges)
                    processed += 1
                except Exception as exc:
                    failures += 1
                    self.report({"ERROR"}, f"{REPORT_PREFIX}: failed to select edge loop candidates on {obj.name}: {exc}")

            for obj in context.view_layer.objects:
                obj.select_set(obj in selected_objects)
            context.view_layer.objects.active = active if active in selected_objects else selected_objects[0]
            bpy.ops.object.mode_set(mode="EDIT")
            bpy.ops.mesh.select_mode(type="EDGE")
        except Exception as exc:
            failures += 1
            self.report({"ERROR"}, f"{REPORT_PREFIX}: failed to show edge loop candidates in Edit Mode: {exc}")
            _restore_context(context, active, selected, mode)

        self.report(
            {"INFO"},
            f"{REPORT_PREFIX}: selected {selected_edge_count} edge loop candidate edge(s) on {processed} object(s), failed {failures}.",
        )
        return {"FINISHED"} if processed and selected_edge_count else {"CANCELLED"}
