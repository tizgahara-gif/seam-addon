...
- Select Edge Loop Candidates for previewing quad-based edge loops that may be useful seam candidates.
...
5. Click one of the action buttons:
   ...
   - **Select Edge Loop Candidates**: selects quad-based edge loop candidate edges and switches to Edit Mode Edge Select for review.
...
### Edge Loop Candidates

- **Edge Loop Min Length**: Minimum number of edges required before a traced edge loop candidate is selected. The default is `8`.
- **Closed Loops Only**: When enabled, open edge loop paths that stop at boundaries, poles, non-manifold edges, triangles, or n-gons are ignored.
...
- **Edge loop review**: Use **Select Edge Loop Candidates** to inspect quad-based loop candidates before manually marking additional seams.
...
Selects edges on visible selected mesh objects that can be traced as quad-based edge loop candidates. The detector follows the opposite edge across quad faces and stops at triangles, n-gons, non-manifold edges, branches, poles, or boundaries. After selection, the add-on switches to Edit Mode with Edge Select enabled so the result can be inspected before marking seams manually.

This tool only changes mesh edge selection; it does not mark seams, unwrap UVs, pack UVs, or alter existing UV maps.
...
- **Select Edge Loop Candidates**: Select a quad mesh, run **Select Edge Loop Candidates**, and verify only traced loop candidate edges are selected in Edit Mode Edge Select.
