...
class AUTOSEAMUV_PG_settings(bpy.types.PropertyGroup):
    ...
    edge_loop_min_length: IntProperty(
        name="Edge Loop Min Length",
        description="Minimum number of edges required for Select Edge Loop Candidates",
        default=8,
        min=1,
        max=10000,
    )

    closed_loops_only: BoolProperty(
        name="Closed Loops Only",
        description="Only select edge loop candidates that form closed loops",
        default=False,
    )
